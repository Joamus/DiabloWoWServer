...
public:
    ...
    static bool HandleParagonStrengthCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        const bool didSetStat = player->SetParagonStrength((uint32) amount, false);
        if (didSetStat) {
            handler->PSendSysMessage("Paragon strength set to %u", (uint32) amount);
        }
        else {
    ...
    static bool HandleParagonAgilityCommand(ChatHandler* handler, char const* args) {
        if (!*args)
            return false;
        ...
        const bool didSetStat = player->SetParagonAgility((uint32) amount, false);
        if (didSetStat) {
            handler->PSendSysMessage("Paragon agility set to %u", (uint32) amount);
        }
        else {
            handler->SendSysMessage("Not enough available paragon points");
            handler->SetSentErrorMessage(true);
    ...
    static bool HandleParagonStaminaCommand(ChatHandler* handler, char const* args) {
        if (!*args)
            return false;
        ...
        const bool didSetStat = player->SetParagonStamina((uint32) amount, false);
        if (didSetStat) {
            handler->PSendSysMessage("Paragon stamina set to %u", (uint32) amount);
        }
        else {
            handler->SendSysMessage("Not enough available paragon points");
            handler->SetSentErrorMessage(true);
    ...
    static bool HandleParagonIntellectCommand(ChatHandler* handler, char const* args) {
        if (!*args)
            return false;
        ...
        const bool didSetStat = player->SetParagonIntellect((uint32) amount, false);
    ...
    static bool HandleParagonSpiritCommand(ChatHandler* handler, char const* args) {
        if (!*args)
            return false;
        ...
        const bool didSetStat = player->SetParagonSpirit((uint32) amount, false);
        if (didSetStat) {
    ...
    static bool HandleParagonSpellPowerCommand(ChatHandler* handler, char const* args) {
        ...
        const bool didSetStat = player->SetParagonSpellPower((uint32) amount, false);
        if (didSetStat) {
    ...
    static bool HandleParagonLifestealCommand(ChatHandler* handler, char const* args) {
        ...
        if ((uint32) amount > _MAX_PARAGON_LIFESTEAL) {
            amount = _MAX_PARAGON_LIFESTEAL;
        }
        ...
        const bool didSetStat = player->SetParagonLifesteal((uint32) amount, false);
        if (didSetStat) {
            std::string s = "";
            ...
            handler->PSendSysMessage("Paragon lifesteal set to %u of %u (%s%%)", amount, sWorld->getIntConfig(MAX_PARAGON_LIFESTEAL), s);
        }
        else {
            handler->SendSysMessage("Not enough available paragon points");
            handler->SetSentErrorMessage(true);
    ...
    static bool HandleParagonResetCommand(ChatHandler* handler, char const* args) {
        ...
        handler->PSendSysMessage("Paragon levels have been reset; all your points have been refunded.");
    ...
    static bool HandleParagonStatsCommand(ChatHandler* handler, char const* args) {
        ...
        handler->PSendSysMessage("XP: %u/%u", player->GetParagonXP(), sObjectMgr->GetParagonXpForLevel(player->GetParagonLevel()));
    ...
    static bool HandleSetMonsterLevelCommand(ChatHandler* handler, char const* args) {
        if (!*args)
            return false;

        int amount = atoi((char*)args);
        ...
        if (player->IsInCombat()) {
            handler->PSendSysMessage("Cannot set monster level during combat.");
            handler->SetSentErrorMessage(true);
            return false;
        }

        if (player->InBattleground()) {
            handler->PSendSysMessage("Cannot set monster level in a battleground.");
            handler->SetSentErrorMessage(true);
            return false;
        }

        if (player->InArena()) {
            handler->PSendSysMessage("Cannot set monster level in arena.");
            handler->SetSentErrorMessage(true);
            return false;
        }

        if (player->GetMap()->IsDungeon()) {
            handler->PSendSysMessage("Cannot set monster level in a dungeon.");
            handler->SetSentErrorMessage(true);
            return false;
        ...
        if (group) {
            if (group->GetLeaderGUID() != player->GetGUID()) {
                handler->PSendSysMessage("Only the group leader can set the monster level.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            for (Group::MemberSlotList::iterator itr = members.begin(); itr != members.end(); ++itr) {
                ...
                if (!target) {
                    handler->PSendSysMessage("Error occured when trying to find all group members - try again later. ");
                    handler->SetSentErrorMessage(true);
                    return false;
                }
    ...
    static bool HandleGetMonsterLevelCommand(ChatHandler* handler, char const* args) {
        Player* player = handler->GetSession()->GetPlayer();

        Group* playerGroup = player->GetGroup();
        if (playerGroup) {
            handler->PSendSysMessage("Group monster level: %u", playerGroup->GetMonsterLevel());
            return true;
        }
        else {
            handler->PSendSysMessage("Monster level: %u", player->GetMonsterLevel());
            return true;
        }

    }

    static bool HandleMonsterLevelCommand(ChatHandler* handler, char const* args) {
        bool didSetMonsterLevel = false;
        if (*args) {
            didSetMonsterLevel = HandleSetMonsterLevelCommand(handler, args);
        }
        if (!didSetMonsterLevel && *args) {
            return false;
        }
        return HandleGetMonsterLevelCommand(handler, args);
    }
