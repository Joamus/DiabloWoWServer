...
void ObjectMgr::LoadItemTemplates()
{
    ...
    //                                           134        135            136          137         138             139
                                             "FoodType, minMoneyLoot, maxMoneyLoot, flagsCustom, monster_level, inherit_from_item FROM item_template");
    ...
    do
    {
        ...
        // For items of higher monster level, they are based on another item e.g Shadowmourne [MLVL 34], so inheritFromItem points to the entry ID of Shadowmourne
        itemTemplate.monsterLevel = fields[138].GetUInt16();
        itemTemplate.inheritFromItem = fields[139].GetUInt32();
...
bool ObjectMgr::LoadParagonXpInfo() {
    QueryResult result = WorldDatabase.Query("SELECT level, xp FROM world.paragon_xp_for_level");
    if (!result)
        return false;
        // SQL Statement
    ...
    do
    {
        ...
        // Put in map
        paragonXpInfo.insert(std::pair<uint32, uint32>(level, xp));
