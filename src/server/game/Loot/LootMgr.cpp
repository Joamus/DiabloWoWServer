...
class LootTemplate::LootGroup                               // A set of loot definitions for items (refs are not allowed)
{
    public:
        ...
        void Process(Loot& loot, uint16 lootMode, uint16 monsterLevel = 1) const;    // Rolls an item from the group (if any) and adds the item to the loot
...
void LootTemplate::LootGroup::Process(Loot& loot, uint16 lootMode, uint16 monsterLevel) const
{
    if (LootStoreItem const* item = Roll(loot, lootMode))
        loot.AddItem(*item, monsterLevel);
}
...
void LootTemplate::Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId, uint16 monsterLevel) const
{
    if (groupId)                                            // Group reference uses own processing of the group
    {
        ...
        Groups[groupId - 1]->Process(loot, lootMode, monsterLevel);
        return;
    }
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        if (item->reference > 0)                            // References processing
        {
            ...
            for (uint32 loop = 0; loop < maxcount; ++loop)      // Ref multiplicator
                Referenced->Process(loot, rate, lootMode, item->groupid, monsterLevel);
        }
        else                                                    // Plain entries (not a reference, not grouped)
            loot.AddItem(*item, monsterLevel);                                // Chance is already checked, just add
    }

    // Now processing groups
    for (LootGroups::const_iterator i = Groups.begin(); i != Groups.end(); ++i)
        if (LootGroup* group = *i)
            group->Process(loot, lootMode, monsterLevel);
}
