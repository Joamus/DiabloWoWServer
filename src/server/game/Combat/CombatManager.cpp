...
bool CombatManager::SetInCombatWith(Unit* who)
{
    ...
    // If PvE combat and the creature is not already in combat, then we want to take the player's monster level and set on the creature.
    // The monster level will buff the stats of said creature, and it will also change which loop will drop.

    if (!isPvP) {
        bool isOwnerPlayer = _owner->IsControlledByPlayer();
        bool isWhoPlayer = who->IsControlledByPlayer();
