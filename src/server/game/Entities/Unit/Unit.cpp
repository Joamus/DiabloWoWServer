...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    {
        // Base value
        DoneAdvertisedBenefit += ToPlayer()->GetBaseSpellPowerBonus(false);
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    {
        // Base value
        advertisedBenefit += ToPlayer()->GetBaseSpellPowerBonus(false);
...
/*static*/ void Unit::Kill(Unit* attacker, Unit* victim, bool durabilityLoss /*= true*/)
{
    ...
    const Creature* creatureFromWorld = victim->GetMap()->GetCreature(victim->GetGUID());
    ...
    if (isRewardAllowed && player && player != victim)
    {
        ...
        if (creature)
        {
            ...
            if (creature->GetLootMode() > 0) {
                ...
                loot->gold *= creatureFromWorld ? creatureFromWorld->GetMonsterLevelGoldMultiplier() : 1;
...
bool Unit::SetMonsterLevel(uint16 monsterLevel) {
    if (monsterLevel > sObjectMgr->GetMaxMonsterLevel()) {
        return false;
    }
    if (this->IsCritter() || !this->IsCreature()) {
        return false;
    }
