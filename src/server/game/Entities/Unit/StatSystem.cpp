...
bool Player::UpdateAllStats()
{
    ...
    for (uint8 i = STAT_STRENGTH; i < MAX_STATS; ++i)
    {
        ...
        /*auto paragonStat = paragonStats.find(Stats(i));
        if (paragonStat->second > 0) {
            value += (int) paragonStat->second;
        }*/
...
void Player::UpdateMaxHealth()
{
    ...
    uint32 extraHealth = value * 0.01f * GetParagonDefense() * 0.04f;
    value += extraHealth;
...
void Creature::UpdateMaxHealth()
{
    ...
    SetMaxHealth(uint32(value * GetMonsterLevelHealthMultiplier()));
}
...
void Creature::UpdateMaxPower(Powers power)
{
    ...
    value *= GetPctModifierValue(unitMod, TOTAL_PCT);
    if (power == POWER_MANA) {
        value *= GetMonsterLevelManaMultiplier();
    }
...
void Creature::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float& minDamage, float& maxDamage, uint8 damageIndex /*= 0*/) const
{
    ...
    float totalValue       = GetFlatModifierValue(unitMod, TOTAL_VALUE);
    float totalPct         = addTotalPct ? GetPctModifierValue(unitMod, TOTAL_PCT) : 1.0f;
    float dmgMultiplier    = GetCreatureTemplate()->ModDamage * GetMonsterLevelDamageMultiplier(); // = ModDamage * _GetDamageMod(rank);
