...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,skin=?,face=?,hairStyle=?,hairColor=?,facialStyle=?,bankSlots=?,restState=?,playerFlags=?,"
                     ...
                     "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,grantableLevels=?,online=?, paragon_strength=?, paragon_agility=?, paragon_stamina=?, paragon_intellect=?, paragon_spirit=?, paragon_level=?, paragon_xp=?, paragon_available_points=?, monster_level=?, paragon_spell_power=?, paragon_lifesteal=?, paragon_offense=?, paragon_defense=?, paragon_heal=? WHERE guid=?", CONNECTION_ASYNC);
